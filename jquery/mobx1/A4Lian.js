...
export default class A4Lian extends Component {
    ...
    _onChange=()=> {
      ...
      $("#id_year").val(store.year);
      $("#id_start").val(store.start);
      $("#id_num").val(store.num);
      $("#id_a4").empty();
      let start=store.start;
      for(var i=0;i<store.num;i++){
        // console.log("append");
        let str_start=sprintf("%04d%04d",store.year,start);
        let lian="第一联　存根联";
  ...
  componentDidMount=()=>{
    ...
    store.set_year(cfg.year);
    store.set_num(cfg.num);
    store.set_start(cfg.start);
    autorun(this._onChange);
    // myredux.ItemActionCreators.load_config(cfg);
  }
  constructor() {
    super();
    ipcRenderer.on("request_close",()=>{
      this.saveconfig({start:store.start,year:store.year,num:store.num});
...
<div>
  <div class="only_screen">
    <div style="display:flex;justify-content:space-between">
       <div>
        <label>起始号码</label><input id="id_start" value={store.start} onChange={this.onChange}></input>
        <label>页数</label><input id="id_num" value={store.num} onChange={this.onChange_num}></input>
        <button  id="id_print" onClick={this.onClick}>打印</button>
       </div>
       <div>
        <label>year</label><input id="id_year" value={store.year} onChange={this.onChange_year}></input>
       </div>
    </div>
  </div>
